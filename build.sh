...
if [ -f "meson.build" ]; then
  ...
  # dnf -y install ncurses-devel flex bison elfutils-libelf-devel dwarves ccache zstd bc rpm-build bpftool gcc-plugin-devel glibc-static perl-devel perl-generators python3-devel elfutils-devel the_silver_searcher
  if [ -e "/boot/dtb" ]; then
