...
run() {
  j=$1
  cmd="cd ~/git/libcamera; rm -rf build; git reset --hard 44d59841e1ce59042b80; meson build -Dqcam=disabled -Dgstreamer=disabled -Dlc-compliance=disabled -Dtest=false -Ddocumentation=disabled -Dpipelines=uvcvideo -Dipas=vimc; time ninja -C build -j$j; echo; grep -m1 'model name\|Hardware' /proc/cpuinfo; $mem; grep -m1 'cores' /proc/cpuinfo; printf 'jobs : $j\\n'"
